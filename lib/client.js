...
function MqttClient (streamBuilder, options) {
  ...
  // # REPLYER
  // Allow autosubscription through simple API
  // but without subscribing to list of events
  // available
  const NON_SUBSCRIBE_EVENTS = [
    'connect', 'reconnect', 'close', 'offline', 'outgoingEmpty',
    'error', 'message', 'newListener', 'removeListener'
  ];

  this.on('newListener', function (event) {
    if (NON_SUBSCRIBE_EVENTS.indexOf(event) === -1) {
...
MqttClient.prototype.publish = function (topic, message, opts, callback) {
  ...
  packet = {
    ...
    qos: opts.qos,
    retain: opts.retain,
    // REPLYER request method reuses publish method with opts
    messageId: opts.messageId || this._nextId()
  };
...
/**
 * request - request <message> to <topic> and get a <callback>
 *
 * @param {String} topic - topic to request to
 * @param {String, Buffer} message - request body
 * @param {Object} [opts] - publish options, includes:
 *    {Number} qos - qos level to publish on
 *    {Boolean} retain - whether or not to retain the message
 * @param {Function} [callback] - function(err, res){}
 *    called when a response is received or fails
 * @returns {MqttClient} this - for chaining
 * @api public
 *
 * @example 
 *     client.request('topic', 'message', {}, function (err, data) {
 *        if (err) throw err
 *        console.log(data)
 *     });
 */

MqttClient.prototype.request = function (topic, message, options, callback) {
  const that = this
    , clientId = this.options.clientId
    , messageId = this._nextId() // override messageId for publish
    , replyUrl = '@' + clientId + '/' + messageId;

  // .request(topic, payload, cb);
  if ('function' === typeof opts) {
    callback = opts;
    opts = null;
  }
  ...
  this.on(replyUrl, function replyHandler (err, data) {
    callback(err, data);
  });
  ...
  const reqTopic = topic + '/@' + clientId + '/' + messageId;
  this.publish(reqTopic, message, options, function (err) {
    if (err) {
      that.removeListener(replyUrl, replyHandler);
      return callback(err);
    }
  });
};
...
MqttClient.prototype._handlePublish = function (packet, done) {
  ...
  switch (qos) {
    ...
    case 0:
      ...
      // # REPLYER
      // Expose such message under a event.
      // Parse message to get rid of JSON.parse
      // or keep it as String
      this._fireHandlers(topic, message, packet);
...
/* Replyer helper function to emit events when a message arrive */
MqttClient.prototype._fireHandlers = function (topic, message, packet) {
  ...
  try {
    ...
    /* do not throw */
    messageParsed = message.toString();
  }
  // The event passes as second argument
  // a function that allows to create a response
  // for that message
  const _reply = this._reply.bind(this, topic, packet);

  Object.keys(this._events).forEach(function (event) {
    const test_event = event.replace('/#', '');
    if (topic.indexOf(test_event) === 0) {
      that.emit(event, messageParsed, _reply);
    }
  });
}

/**
 * _reply sends a packet to a certain
 * @client/packet_id, packet will be binded
 * by _handlePublish
 */
MqttClient.prototype._reply = function (topic, packet, data) {
  const path_arr = topic.split('/');
  const issuer = path_arr[path_arr.length - 2];
  const dest = issuer + '/' + path_arr[path_arr.length - 1];
  this.publish(dest, JSON.stringify(data));
};
