...
function MqttClient (streamBuilder, options) {
  ...
  // available
  const NON_SUBSCRIBE_EVENTS = [ 'connect', 'reconnect', 
    'close', 'offline', 'outgoingEmpty', 'error',
    'message', 'newListener', 'removeListener'
  ];
 ...
 *
 * @example 
 *     client.request('topic', {JSON: body}, function (err, data) {
 *        if (err) throw err
 *        console.log(data)
 *     });
 */

MqttClient.prototype.request = function (topic, message, opts, callback) {
  const that = this
  ...
  this.on(replyUrl, function replyHandler (data, topic) {
    that.removeListener(replyUrl, replyHandler);
    return callback(null, data, topic);
  });
  ...
  const _done = callback

  const reqTopic = topic + '/@' + clientId + '/' + messageId;
  this.publish(reqTopic, message, opts, function (err) {
    if (err) {
...
MqttClient.prototype._fireHandlers = function (topic, message, packet) {
  ...
  Object.keys(this._events).forEach(function (event) {
    ...
    if (topic.indexOf(test_event) === 0) {
      that.emit(event, messageParsed, topic, packet);
    }
  });
}
 ...
 * by _handlePublish
 */
MqttClient.prototype.reply = function (topic, data) {
